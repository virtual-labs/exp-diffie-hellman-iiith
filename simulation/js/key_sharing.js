...
function nextPrime() {
  currentPrimeIndex = (currentPrimeIndex + 1) % primes.length;
  currentPrime = primes[currentPrimeIndex];
  document.getElementById("prime").value = currentPrime;
  currentGeneratorIndex = generators[currentPrimeIndex].length - 1;
  // Automatically set the first generator for this prime
...
function nextGenerator() {
  if (currentPrimeIndex >= 0) {
    currentGeneratorIndex =
      (currentGeneratorIndex - 1 + generators[currentPrimeIndex].length) %
      generators[currentPrimeIndex].length;
    currentGenerator = generators[currentPrimeIndex][currentGeneratorIndex];
    document.getElementById("generator").value = currentGenerator;
  }
}
...
function nextKeyA() {
  if (currentPrimeIndex >= 0) {
    document.getElementById("keyA").value =
      Math.ceil(Math.random() * 1000000) % primes[currentPrimeIndex];
  }
}

function calculateGA() {
  var a = document.getElementById("keyA").value;
  document.getElementById("encryptA").value = powAndMod(
    currentGenerator,
    a,
    currentPrime
  );
...
function sendA() {
  document.getElementById("receivedB").value =
    document.getElementById("encryptA").value;
}
...
function calculateGB() {
  ...
  document.getElementById("encryptB").value = powAndMod(
    currentGenerator,
    b,
    currentPrime
  );
...
function sendB() {
  document.getElementById("receivedA").value =
    document.getElementById("encryptB").value;
}
